...
def chroma_questions(slide, max_q=3):
    ...
    1. Try exact Chroma slide text
    2. If empty → fallback to slide image (GPT-4.1 vision)
    """
...
"""
        ...
        img_path = slide.get("png_path")
        if not img_path or not os.path.exists(img_path):
            raise RuntimeError("No slide text or image available")
...
TASK:
Generate up to {max_q} clear, specific questions
to help customize this slide.

Rules:
- Questions must be based on visible content
- No generic questions
- Plain numbered text only
"""
                    },
...
for slide in slides:
    ...
    if slide_type == "title":
        ...
        questions = ["What should be included in the agenda?"]
