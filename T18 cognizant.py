...
def fill_content_body(slide, bullets):
    ...
    for i, bullet in enumerate(bullets):
        ...
        p.text = bullet
        p.level = 0            # ✅ FIX: first bullet now bullet
        p.font.size = Pt(20)


# ------------------------------------------------------------
# CONTENT TITLE (KEEP WHITE)
# ------------------------------------------------------------
def set_content_title(slide, title):
    ...
    tf = slide.shapes.title.text_frame
    p = tf.paragraphs[0]      # ❗ DO NOT clear → preserves color
    p.text = title
...
def generate_presentation_cognizant(payload):
    ...
    for slide_data in slides[1:]:
        ...
        update_footer(slide)   # ✅ FIXED: footer now appears
