...
def resolve_slide_title(slide):
    """
    Preferred title resolution:
    1. Extracted slide title
    2. Fallback to Chroma DB (first meaningful line)
    """
    ...
    query = f"Slide {slide_index}"

    results = semantic_search(query=query, top_k=1)
    if results:
...
for idx, slide in enumerate(slides):
    ...
    resolved_title = resolve_slide_title(slide)

    st.markdown("---")
    st.subheader(f"Slide {idx+1}: {resolved_title}")
